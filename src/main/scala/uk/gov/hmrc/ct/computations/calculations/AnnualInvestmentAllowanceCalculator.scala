...
trait AnnualInvestmentAllowanceCalculator {
  ...
  def maximum(cp1: CP1, cp2: CP2, allowableAmounts: Set[AnnualInvestmentAllowancePeriod]): CATO02 = {
    val maximumEntitlement = maximumEntitlementForAccountingPeriod(cp1, cp2, allowableAmounts) _

    CATO02(
      if(consistsOfCalendarMonths(cp1, cp2)) maximumEntitlement(false) max maximumEntitlement(true) else maximumEntitlement(false)
    )
  }
  ...
  protected def yearLengthForDailyBasis(accountingPeriodStartDate: LocalDate, accountingPeriodEndDate: LocalDate): Int =
    AnnualInvestmentAllowancePeriods().filter(_.startDateFallsWithin(accountingPeriodStartDate, accountingPeriodEndDate)) match {
      case annualInvestmentAllowancePeriodsStartingInAccountingPeriod if annualInvestmentAllowancePeriodsStartingInAccountingPeriod.nonEmpty => DaysInYear
      case _ => {
  ...
  private def maximumEntitlementForAccountingPeriod(cp1: CP1, cp2: CP2, allowableAmounts: Set[AnnualInvestmentAllowancePeriod])(useMonthlyBasis: Boolean): Int = {
    val yearLengthForBasis = if(useMonthlyBasis) MonthsInYear else yearLengthForDailyBasis(cp1.value, cp2.value)

    allowableAmounts.toList.map { investmentAllowancePeriod =>
      val intersectingPeriod = if(useMonthlyBasis)
        investmentAllowancePeriod.intersectingPeriod(cp1.value, cp2.value, monthsBetween)
      else
        investmentAllowancePeriod.intersectingPeriod(cp1.value, cp2.value, daysBetween)

      val proportionOfAIAPeriod = BigDecimal(intersectingPeriod) / BigDecimal(yearLengthForBasis)

      val proRatedForIntersectingPeriod = BigDecimal(investmentAllowancePeriod.maximumAllowed) * proportionOfAIAPeriod
